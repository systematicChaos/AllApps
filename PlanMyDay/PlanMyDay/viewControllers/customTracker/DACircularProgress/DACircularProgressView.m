...
- (id)init
{
    return [super initWithFrame:CGRectMake(0.0f, 20.0f, 40.0f, 40.0f)];
}
...
- (void)stopAnimation
{
    CFTimeInterval paused_time = [self.circularProgressLayer convertTime:CACurrentMediaTime() fromLayer:nil];
    self.circularProgressLayer.speed = 0.0;
    self.circularProgressLayer.timeOffset = paused_time;
}
-(void)resumeAnimation
{
    CFTimeInterval paused_time = [self.circularProgressLayer timeOffset];
    self.circularProgressLayer.speed = 1.0f;
    self.circularProgressLayer.timeOffset = 0.0f;
    self.circularProgressLayer.beginTime = 0.0f;
    CFTimeInterval time_since_pause = [self.circularProgressLayer convertTime:CACurrentMediaTime() fromLayer:nil] - paused_time;
    self.circularProgressLayer.beginTime = time_since_pause;
}

- (void)setAnimatedProgress:(CGFloat)progress withDuration:(float)duration
{
